...
def extract_markdown_images(text):
    image_re = r"!\[([a-zA-Z\d\s]+)\]\(([a-zA-Z\d\s:/\.@]+)\)"
    matches = re.findall(image_re, text)
    return matches

def extract_markdown_links(text):
    link_re = r"\[([a-zA-Z\d\s]+)\]\(([a-zA-Z\d\s:/\.@]+)\)"
...
def text_to_textnodes(text):
    temp_textnode = TextNode(text, TextType.PLAIN_TEXT)
    new_nodes = split_nodes_delimiter([temp_textnode], "`", TextType.CODE_TEXT)
